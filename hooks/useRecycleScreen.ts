...
export function useRecycleScreen<T extends AnyItem>(type: Kind) {
  ...
  }, [type, stickiesMap, todosMap, remindersMap, deletedByType]);
  ...
  const toggleSelectAll = useCallback(() => {
    ...
    setSelectedIds((prev) =>
      prev.size === visibleCount ? new Set() : new Set(data.map((n: any) => n.id))
    );
  }, [data, visibleCount]);
  ...
  const performDeleteForever = useCallback(() => {
    ...
    exitSelection();
  }, [pendingIds, type, purgeNote, purgeSticky, purgeTodo, purgeReminder, exitSelection]);
  ...
  const performRestoreSelected = useCallback(async () => {
    ...
    setSnackOpen(true);
    exitSelection();
  }, [selectedIds, type, restoreSticky, restoreTodo, restoreReminder, restoreNote, exitSelection]);
