...
type AddPayload = { title: string; place?: string; remindAt: string; repeat?: Repeat };
...
export const useReminderStore = create<ReminderState>((set, get) => ({
  ...
  addReminder: ({ title, place, remindAt, repeat = 'none' }) => {
    ...
    const maxOrder = current.length ? Math.max(...current.map(r => r.order ?? 0)) : -1;
  ...
  updateReminder: (id, patch) =>
    set(s => {
      const prev = s.reminders[id]; if (!prev) return s as any;
  ...
  softDelete: (id) =>
    set(s => {
      const prev = s.reminders[id]; if (!prev || prev.deletedAt) return s as any;
      return { reminders: { ...s.reminders, [id]: { ...prev, deletedAt: new Date().toISOString(), updatedAt: new Date().toISOString() } } };
    }),
  ...
  restore: (id) =>
    set(s => {
      ...
      return { reminders: { ...s.reminders, [id]: { ...rest, due, updatedAt: new Date().toISOString() } } };
    }),

  hardDelete: (id) =>
    set(s => { const next = { ...s.reminders }; delete next[id]; return { reminders: next }; }),

  reorder: (ids) =>
    set(s => {
      const next = { ...s.reminders };
      ids.forEach((id, idx) => {
        const r = next[id]; if (r) next[id] = { ...r, order: idx, updatedAt: r.updatedAt };
      });
