...
const withHistory = (
  prev: NoteContentState,
  next: Partial<NoteContentState>
): NoteContentState => {
...
const getActive = (s: NoteContentState) => {
  const bi = Math.min(
    Math.max(s.selection?.blockIndex ?? 0, 0),
    Math.max(s.blocks.length - 1, 0)
  );
...
// commands
export const setTitle = (s: NoteContentState, title: string) =>
  withHistory(s, { title });
export const toggleEditing = (s: NoteContentState) => ({
  ...s,
  isEditing: !s.isEditing,
});
export const setSelection = (s: NoteContentState, sel: Selection) => ({
  ...s,
  selection: sel,
});
export const setPageTemplate = (s: NoteContentState, pt: PageTemplate) =>
  withHistory(s, { pageTemplate: pt });
export const setBackgroundColor = (s: NoteContentState, hex: string) =>
  withHistory(s, { backgroundColor: hex });
...
export const setFontSize = (s: NoteContentState, size: number | null) => {
  ...
  if (block.type === "bullet")
    block.items = block.items.map((it) => ({ ...it, marks: apply(it.marks) }));
...
export const applyMark = (s: NoteContentState, patch: Partial<TextMark>) => {
  ...
  if (block.type === "bullet")
    block.items = block.items.map((it) => ({ ...it, marks: merge(it.marks) }));
  if (block.type === "checkbox")
    block.items = block.items.map((it) => ({ ...it, marks: merge(it.marks) }));
  return withHistory(s, { blocks: next.blocks });
};

export const setColor = (s: NoteContentState, color: string | null) =>
  applyMark(s, { color });
export const setHighlight = (s: NoteContentState, highlight: string | null) =>
  applyMark(s, { highlight });

export const toggleBold = (s: NoteContentState) =>
  applyMark(s, { bold: !currentBool(s, "bold") });
export const toggleItalic = (s: NoteContentState) =>
  applyMark(s, { italic: !currentBool(s, "italic") });
export const toggleUnderline = (s: NoteContentState) =>
  applyMark(s, { underline: !currentBool(s, "underline") });

function currentBool(
  s: NoteContentState,
  key: keyof Pick<TextMark, "bold" | "italic" | "underline">
) {
  const { block } = getActive(s);
...
export const toggleBulletList = (s: NoteContentState) => {
  ...
  if (block.type === "paragraph") {
    next.blocks[bi] = {
      type: "bullet",
      items: [{ text: block.text, marks: block.marks }],
      align: block.align,
    };
...
export const toggleCheckboxList = (s: NoteContentState) => {
  ...
  if (block.type === "paragraph") {
    next.blocks[bi] = {
      type: "checkbox",
      items: [{ checked: false, text: block.text, marks: block.marks }],
      align: block.align,
    };
  } else if (block.type === "checkbox") {
    const text = block.items.map((i) => i.text).join("\n");
...
export const editParagraphText = (
  s: NoteContentState,
  bi: number,
  text: string
) => {
...
export const editBulletItem = (
  ...
  if (b?.type === "bullet") {
    ...
    return withHistory(s, {
      blocks: next.blocks,
      selection: { blockIndex: bi, itemIndex: idx },
    });
  }
  return s;
};

export const editCheckboxItem = (
  s: NoteContentState,
  bi: number,
  idx: number,
  changes: Partial<{ checked: boolean; text: string }>
) => {
  const next = clone(s);
  const b = next.blocks[bi];
  if (b?.type === "checkbox") {
...
export const addListItemAfter = (s: NoteContentState) => {
  ...
  const b = next.blocks[bi];
  if (b?.type === "bullet") {
    b.items.splice((next.selection?.itemIndex ?? b.items.length - 1) + 1, 0, {
      text: "",
      marks: b.items[0]?.marks,
    });
    return withHistory(s, {
      blocks: next.blocks,
      selection: {
        blockIndex: bi,
        itemIndex: (next.selection?.itemIndex ?? 0) + 1,
      },
    });
  }
  if (b?.type === "checkbox") {
