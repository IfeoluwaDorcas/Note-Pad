...
export default function NotesToolbar(props: Props) {
    ...
    <>
        ...
        items={
          mode === 'recycle'
            ? [
                ...
                { key: 'empty', label: 'Empty bin', onPress: () => onEmptyBin?.() },
              ]
            : [
                {
                  key: 'edit',
      ...
      {mode !== 'recycle' && enableViewMenu && (
        <PopoverMenu
          ...
          anchorRef={t.moreAnchorRef}
          items={[
            { key: 'gridS', label: radio('Grid (small)', t.view === 'gridS'), onPress: () => t.setView('gridS') },
            { key: 'gridM', label: radio('Grid (medium)', t.view === 'gridM'), onPress: () => t.setView('gridM') },
            { key: 'gridL', label: radio('Grid (large)', t.view === 'gridL'), onPress: () => t.setView('gridL') },
          ]}
        />
      )}
    </>
  ...
  if (variant === 'sticky') {
    return (
      <Animated.View
        style={[styles.stickyWrap, t.stickyFadeStyle, { backgroundColor: T.colors.bg }]}
        pointerEvents={t.searchOpen ? 'auto' : t.compactVisible ? 'auto' : 'none'}
      >
        <View style={styles.row1}>
          ...
          <View style={styles.stickyCenter}>
            {!t.searchOpen && (
              <Text numberOfLines={1} style={[styles.stickyTitleLeft, { color: T.colors.text }]}>
                {title}
              </Text>
            )}
...
function IconBtn({ onPress, children, style }: { onPress?: () => void; children: React.ReactNode; style?: any }) {
  return (
    <Pressable
      onPress={onPress}
      hitSlop={8}
      accessibilityRole="button"
      style={({ pressed }) => [{ padding: 8, borderRadius: 10, opacity: pressed ? 0.7 : 1 }, style]}
    >
      {children}
    </Pressable>
  );
...
const styles = StyleSheet.create({
  ...
  row1: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 12 },
  centerSearch: { flex: 1, height: 36, position: 'relative', justifyContent: 'center' },
  ...
  stickyCenter: { flex: 1, height: 36, justifyContent: 'center', marginLeft: 8, marginRight: 8 },
  ...
  row2: { flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end', marginTop: 5, paddingHorizontal: 12, gap: 8 },
  allToggle: { flexDirection: 'row', alignItems: 'center', gap: 6, padding: 8, borderRadius: 10 },
